...
class BooksDownloads extends StatefulWidget {
  @override
  _BooksDownloadsState createState() => _BooksDownloadsState();
}

class _BooksDownloadsState extends State<BooksDownloads> {

List<BooksData>? bookListDownload;
  ...
  void getDataBook(int count, String strSearch) async {
    loadingList = true;
    setState(() {});
    List arr = await getData(count, "books/readbook_user_download.php", strSearch, "user_id=${G_user_id_val}&");
    for (int i = 0; i < arr.length; i++) {
      bookListDownload!.add(BooksData(
rep_id: arr[i]["rep_id"],
        ...
        book_Number_of_reviews: 12,
         book_size: (arr[i]["book_size"]).toString(),
       
       
        book_file: arr[i]["book_file"], //
      ));
      List avg = await getData(count, "evas/readeva_avg.php", strSearch,
            "book_id=${bookListDownload![i].book_id}&");
      if(avg[0]["eva_avg"] !=null){
            bookListDownload![i].book_eva = double.parse(avg[0]["eva_avg"]);
      }
       if(avg[0]["eva_count"] !=null){
            bookListDownload![i].book_Number_of_reviews = int.parse(avg[0]["eva_count"]);
      }
    }
    loadingList = false;
    setState(() {});
              ...
              children: [
                Container(
                  ...
                  child: ListView.builder(
                    controller: myScroll,
                    itemCount: bookListDownload!.length,
                    itemBuilder: (context, index) {
                      final item = bookListDownload![index];
                      return Dismissible(
                        key: Key(item.book_id),
                        direction: DismissDirection.startToEnd,
                        child: SingleBookAll(
                          book_index: index,
                          books: bookListDownload![index],
                        ),
                        onDismissed: (direction) {
                          bookListDownload!.remove(item);
                          // deleteData(
