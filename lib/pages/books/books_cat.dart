...
import 'package:digital_books/pages/books/add_book.dart';
  ...
  void getDataBook(int count, String strSearch) async {
    ...
    for (int i = 0; i < arr.length; i++) {
      ...
      List avg = await getData(count, "evas/readeva_avg.php", strSearch,
          "book_id=${bookListCat![i].book_id}&");
      if (avg[0]["eva_avg"] != null) {
        bookListCat![i].book_eva = double.parse(avg[0]["eva_avg"]);
      }
      if (avg[0]["eva_count"] != null) {
        bookListCat![i].book_Number_of_reviews = int.parse(avg[0]["eva_count"]);
      }
    }
    loadingList = false;
    setState(() {});
              ...
              children: [
                Container(
                  ...
                  child: ListView.builder(
                    controller: myScroll,
                    itemCount: bookListCat!.length,
                    itemBuilder: (context, index) {
                      final item = bookListCat![index];
                      return Dismissible(
                        key: Key(item.book_id),
                        direction: DismissDirection.startToEnd,
                        child: SingleBookAll(
                          book_index: index,
                          books: bookListCat![index],
                        ),
                        onDismissed: (direction) {
                          bookListCat!.remove(item);
                          // deleteData(
