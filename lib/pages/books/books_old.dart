...
List<BooksData>? bookListold;

class BooksOld extends StatefulWidget {
  @override
  _BooksOldState createState() => _BooksOldState();
}

class _BooksOldState extends State<BooksOld> {
  ...
  void getDataBook(int count, String strSearch) async {
    loadingList = true;
    setState(() {});
    List arr = await getData(count, "books/readbook_user_old.php", strSearch,
        "user_id=${G_user_id_val}&");
    for (int i = 0; i < arr.length; i++) {
      bookListold!.add(BooksData(
        eva_id: arr[i]["eva_id"],
      ...
      List avg = await getData(count, "evas/readeva_avg.php", strSearch,
            "book_id=${bookListold![i].book_id}&");
      
           if(avg[0]["eva_avg"] !=null){
            bookListold![i].book_eva = double.parse(avg[0]["eva_avg"]);
      }
       if(avg[0]["eva_count"] !=null){
            bookListold![i].book_Number_of_reviews = int.parse(avg[0]["eva_count"]);
      }
    }
    loadingList = false;
    setState(() {});
              ...
              children: [
                Container(
                  ...
                  child: ListView.builder(
                    controller: myScroll,
                    itemCount: bookListold!.length,
                    itemBuilder: (context, index) {
                      final item = bookListold![index];
                      return Dismissible(
                        key: Key(item.book_id),
                        direction: DismissDirection.startToEnd,
                        child: SingleBookAll(
                          book_index: index,
                          books: bookListold![index],
                        ),
                        onDismissed: (direction) {
                          bookListold!.remove(item);
                          // deleteData(
